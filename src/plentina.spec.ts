...
describe('PlentinaController', () => {
  ...
  describe('root', () => {
    it('should return "Gaurav Reddy"', async () => {
      //check if health check is working

      expect(plentinaController.healthCheck().name).toBe('Gaurav Reddy');
    });
  });
});
...
//testcase for line collision with circle
describe('PlentinaService', () => {
  let plentinaService: PlentinaService;
